...
const clubs: Club[] = [
  { id: 1, name: '机器人创新组织', shortName: 'R', category: '科技', slogan: '把想法装进机器，让它真正动起来。', members: 186, activity: '机器人开放工坊', location: '工程训练中心 A302', colors: 'from-[#0f766e] to-[#34d399]' },
  { id: 2, name: '银河摄影社', shortName: 'P', category: '艺术', slogan: '记录社区里被匆忙错过的光。', members: 243, activity: '夏至社区夜拍', location: '线上分享厅', colors: 'from-[#4338ca] to-[#a78bfa]' },
  ...
  { id: 4, name: '星火志愿服务队', shortName: 'V', category: '公益', slogan: '让每一次行动都留下真实改变。', members: 312, activity: '社区数字助老', location: '线上活动大厅', colors: 'from-[#be123c] to-[#fb7185]' },
  { id: 5, name: '青年经济学社', shortName: 'E', category: '学术', slogan: '从社区出发，理解真实世界如何运转。', members: 96, activity: '城市经济圆桌', location: '线上会议室 B204', colors: 'from-[#0369a1] to-[#38bdf8]' },
...
const events = [
  { day: '21', weekday: '今天', time: '18:30', title: '机器人开放工坊', club: '机器人创新组织', place: '工程训练中心 A302', tag: '可直接参加', color: 'bg-accent' },
  { day: '22', weekday: '周一', time: '19:00', title: '夏至社区夜拍', club: '银河摄影社', place: '线上分享厅', tag: '剩余 12 席', color: 'bg-violet' },
  { day: '24', weekday: '周三', time: '17:20', title: '飞盘新手体验局', club: '飞盘运动社', place: '东区体育场', tag: '器材免费', color: 'bg-info' },
  { day: '27', weekday: '周六', time: '09:00', title: '社区数字助老', club: '星火志愿服务队', place: '线上活动大厅集合', tag: '需报名', color: 'bg-danger' }
];
...
function SearchBox({ placeholder = '搜索组织或活动' }: { placeholder?: string }) {
...
function PageHeader({ compact = false }: { compact?: boolean }) {
  return (
    <header className={`flex flex-wrap items-center justify-between gap-4 border-b border-divider bg-content px-7 ${compact ? 'py-4' : 'py-6'} max-sm:px-4`}>
      <div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-accent-strong">Organization hall</p><h1 className="mt-1 text-2xl font-semibold tracking-tight text-strong">组织广场</h1>{!compact && <p className="mt-1 text-sm text-muted">从一次活动开始，找到愿意长期参与的社区共同体。</p>}</div>
      <SearchBox />
    </header>
  );
}
  ...
  return (
    <div className="min-h-screen bg-content">
      ...
      <main className="mx-auto max-w-7xl px-7 py-7 max-sm:px-4">
        ...
        <div className="mt-7 flex items-end justify-between"><div><h2 className="text-lg font-semibold text-strong">{category === '全部' ? '值得认识的组织' : `${category}组织`}</h2><p className="mt-1 text-xs text-muted">根据近期活跃度与社区关注推荐</p></div><span className="text-xs text-faint">{visible.length} 个结果</span></div>
