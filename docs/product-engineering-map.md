...
chat-room 当前主线是**组织中心交流平台**。

它不再是高校教学协作平台，也不是泛用聊天室。学校、课程、班级、作业等概念已经归档为历史方向；当前模型以 Organization、Membership、Channel、Activity 为核心。

一句话概括：

> 用户发现组织、加入组织，并在自己已加入组织的默认频道中进行可信权限下的实时交流。

## 2. 核心矛盾

很多组织的真实协作不是缺一个聊天窗口，而是缺一个能同时表达以下关系的系统：

- 组织是什么，为什么值得加入；
- 用户和组织之间是否存在 Membership；
- 频道属于哪个 Organization；
- 用户是否因为 Membership 获得 Channel access；
- 活动如何展示组织的存在感；
- 在线状态、历史消息、未读提醒如何在真实后端中保持可用。

所以本项目要解决的问题是：

> 把组织发现、成员关系、频道访问权和实时交流收束到一条可信主链路，而不是继续把结构化组织协作塞进松散群聊。

## 3. 当前用户角色

### 3.1 User

平台用户。可以注册、登录、发现公开组织、加入组织、进入自己有权访问的组织频道交流。

### 3.2 Organizer

组织维护者。MVP 语义上负责维护组织资料、活动、成员入口和申请码。当前代码只部分支持该角色，完整管理能力仍是 MVP 缺口。
